...
function Sparkline({ data, t, target }) {
  ...
  useEffect(() => {
    ...
    if (pts.length < 2) {
      ctx.fillStyle = t.textFaint; ctx.font = '11px system-ui'; ctx.textAlign = 'center'
    ...
    gridVals.forEach(v => {
      ...
      ctx.fillStyle = t.textFaint; ctx.font = '9px system-ui'; ctx.textAlign = 'right'
    ...
    pts.forEach((d, i) => {
      if (i === 0 || i === pts.length - 1 || i === Math.floor(pts.length / 2)) {
        ...
        ctx.fillStyle = t.textFaint; ctx.font = '9px system-ui'; ctx.textAlign = 'center'
    ...
    pts.forEach((d, i) => {
      ...
      // Label only first, last, and max
      const isLast = i === pts.length - 1
      const isMax = parseFloat(d.value) === Math.max(...vals)
      if (isLast || isMax) {
        ctx.fillStyle = isLast ? '#fff' : '#52E8A0'
...
export default function Home({ theme, t, onNavigate, onRegister, user, profile, lang = 'en' }) {
  ...
  // Dynamic swing target from goals if available
  ...
  const recentEntries = entries.filter(e => e.entry_date && new Date(e.entry_date) >= thirtyDaysAgo)
  const recentSessions = [...new Set(recentEntries.map(e => e.entry_date))].length
  // Golf sessions = days with swing_speed, carry, smash, stack entries
  const golfMetrics = ['swing_speed', 'smash_factor', 'carry', 'stack_speed']
  const gymMetrics = ['deadlift', 'medball', 'thoracic']
  const golfSessions = [...new Set(recentEntries.filter(e => golfMetrics.includes(e.metric_id)).map(e => e.entry_date))].length
  const gymSessions = [...new Set(recentEntries.filter(e => gymMetrics.includes(e.metric_id)).map(e => e.entry_date))].length
  ...
  const gymCards = [
    ...
    { id: 'medball', label: 'MED BALL', unit: 'm' },
    { id: 'thoracic', label: 'MOBILIDADE', unit: '°' },
  ].filter(k => lastMetrics[k.id])
  ...
  const todayDayPT = DAYS_PT[todayDate.getDay()]
  ...
  const getWeekSessions = (plan) => {
    ...
    return sessions.sort((a, b) => DAYS_ORDER.indexOf(a.day) - DAYS_ORDER.indexOf(b.day))
  }
  ...
  return (
    <div style={{ fontFamily: F, color: t.text }}>
        ...
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '16px', marginBottom: '14px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', marginBottom: '12px' }}>
            {[['hcp','Handicap'],['wagr','WAGR'],['club',lang==='pt'?'Clube':'Club'],['category',lang==='pt'?'Categoria':'Category'],['fed',lang==='pt'?'Federação':'Federation'],['fed_num',lang==='pt'?'Nº Federado':'Fed. No.']].map(([k,l]) => (
              <div key={k}><div style={{ fontSize: '8px', color: t.textFaint, marginBottom: '3px', letterSpacing: '1px' }}>{l.toUpperCase()}</div>
        ...
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '14px 18px', marginBottom: '14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
            {[
              ...
              { label: lang==='pt'?'Nº FED':'FED #', value: athlete.fed_num || '—', color: t.text },
            ].map(item => (
              <div key={item.label}>
                <div style={{ fontSize: '8px', letterSpacing: '2px', color: t.textFaint, marginBottom: '3px', fontWeight: 600 }}>{item.label}</div>
                <div style={{ fontSize: '15px', fontWeight: 900, color: item.color, letterSpacing: '-0.3px' }}>{item.value}</div>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: '6px' }}>
            <button onClick={() => { setAthleteForm({...athlete}); setEditingAthlete(true) }} style={{ background: 'transparent', border: `1px solid ${t.border}`, borderRadius: '6px', color: t.textFaint, padding: '5px 10px', fontSize: '10px', cursor: 'pointer', fontFamily: F, letterSpacing: '1px' }}>EDITAR</button>
            <a href="https://www.wagr.com/playerprofile/francisca-salgado-43158" target="_blank" rel="noreferrer" style={{ background: 'transparent', border: `1px solid ${t.border}`, borderRadius: '6px', color: t.textFaint, padding: '5px 10px', fontSize: '10px', cursor: 'pointer', fontFamily: F, textDecoration: 'none', letterSpacing: '1px' }}>WAGR ↗</a>
            <a href="https://portal.fpg.pt/handicaps-course-rating/pesquisa-de-handicaps/" target="_blank" rel="noreferrer" style={{ background: 'transparent', border: `1px solid ${t.border}`, borderRadius: '6px', color: t.textFaint, padding: '5px 10px', fontSize: '10px', cursor: 'pointer', fontFamily: F, textDecoration: 'none', letterSpacing: '1px' }}>FPG ↗</a>
          </div>
        </div>
      )}
      ...
      <div className="home-grid" style={{ marginBottom: '14px' }}>
        {/* Big focus card with proper sparkline */}
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '22px' }}>
          <div style={{ fontSize: '8px', letterSpacing: '4px', color: t.textFaint, marginBottom: '12px', fontWeight: 600 }}>{lang==='pt'?'DRIVE VELOCITY — OBJECTIVO PRINCIPAL':'DRIVE VELOCITY — MAIN GOAL'}</div>
          ...
          {kpiCards.length > 0 && (
            <div className="kpi-golf">
              {kpiCards.map(k => {
                ...
                return (
                  <div key={k.id} onClick={() => onNavigate && onNavigate('performance')} style={{ cursor: 'pointer' }}>
          ...
          {/* Trend */}
          {delta !== null && (
            <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #1e1e1e', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {lastDate && <div style={{ fontSize: '10px', color: t.textFaint, letterSpacing: '1px' }}>{formatDate(lastDate)}</div>}
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {/* Sessions golf + gym separate */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '16px' }}>
              <div style={{ fontSize: '8px', letterSpacing: '3px', color: '#378ADD', marginBottom: '8px', fontWeight: 600 }}>{lang==='pt'?'GOLFE 30D':'GOLF 30D'}</div>
              <div style={{ fontSize: '38px', fontWeight: 900, color: t.text, lineHeight: 1, letterSpacing: '-2px' }}>{golfSessions}</div>
              <div style={{ fontSize: '10px', color: t.textFaint, marginTop: '4px' }}>sessões</div>
            </div>
            <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '16px' }}>
              <div style={{ fontSize: '8px', letterSpacing: '3px', color: '#52E8A0', marginBottom: '8px', fontWeight: 600 }}>{lang==='pt'?'GINÁSIO 30D':'GYM 30D'}</div>
          ...
          <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '18px', flex: 1 }}>
            ...
            {upcomingEvents.length === 0 ? (
              <div style={{ fontSize: '12px', color: t.textFaint, fontStyle: 'italic' }}>Sem eventos próximos</div>
            ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', marginBottom: '14px' }}>
        ...
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '14px 16px' }}>
          ...
          {weekSessions.length === 0 ? (
            <div style={{ fontSize: '12px', color: t.textFaint, fontStyle: 'italic' }}>{lang==='pt'?'Sem plano para esta semana':'No plan for this week'}</div>
          ) : (
          ...
          {todaySession && (
            <div style={{ marginTop: '8px', fontSize: '10px', color: '#378ADD', fontWeight: 600 }}>
              Hoje: {todaySession.detail}
            </div>
          )}
        ...
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '14px 16px' }}>
          ...
          {compStats.length === 0 ? (
            <div style={{ fontSize: '12px', color: t.textFaint, fontStyle: 'italic' }}>{lang==='pt'?'Sem competições registadas':'No competitions recorded'}</div>
          ) : (
        ...
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '14px 16px' }}>
          ...
          {recentResults.length === 0 ? (
            <div style={{ fontSize: '12px', color: t.textFaint, fontStyle: 'italic' }}>{lang==='pt'?'Sem resultados':'No results yet'}</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0px' }}>
      ...
      {gymCards.length > 0 && (
        <div className="kpi-row">
          {gymCards.map(k => {
            ...
            return (
              <div key={k.id} onClick={() => onNavigate && onNavigate('performance')}
                style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '16px', cursor: 'pointer' }}>
